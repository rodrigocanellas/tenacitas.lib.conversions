#ifndef TENACITAS_CONVERSIONS_ALG_CONVERT_H
#define TENACITAS_CONVERSIONS_ALG_CONVERT_H
...
std::optional<t_to> cvt(t_from p_from);
...
template <typename t_to> std::optional<t_to> to_uint(const char *p_str) {
  ...
  while (*p_str) {
    ...
    if ((_result != 0) && ((_result * 10) == 0)) {
      return {};
    }

    _result *= 10;

    if ((_result != 0) && ((_result + _digit) == 0)) {
      return {};
    }
...
//      if (sizeof(t_to) < 8) {
//        if (static_cast<int64_t>(_result * 10) <
//            std::numeric_limits<t_to>::min()) {
//          return {};
//        }
//      } else if ((_result * 10) <= std::numeric_limits<t_to>::max()) {
//        return {};
//      }
...
//      if (sizeof(t_to) < 8) {
//        if (static_cast<int64_t>(_result - _digit) <
//            std::numeric_limits<t_to>::min()) {
//          return {};
//        }
//      } else if ((_result - _digit) <= std::numeric_limits<t_to>::max()) {
//        return {};
//      }

//      _result -= _digit; // calculate in negatives to support INT_MIN,
//      LONG_MIN,..
...
//   return static_cast<uint8_t>(atoi(reinterpret_cast<const char *>(p_from)));
// }
...
//   return static_cast<uint16_t>(atoi(reinterpret_cast<const char *>(p_from)));
// }
...
//   return static_cast<uint32_t>(atol(reinterpret_cast<const char *>(p_from)));
// }
...
// template <typename t_type = std::byte> void cvt(uint8_t p_from, t_type *p_to)
// {
//   *p_to = static_cast<t_type>(p_from);
// }
...
/////
///// \param p_tuple is the tuple that will contain the converted values of the
///// container
/////
///// \param p_ite is a iterator to the container that contains the values to be
///// converted
/////
